var content_controller = function ($scope, $timeout, $sce) {
    ...
    const API_URL = "/wiz/admin/routes/api";
    const TABS = ['controller', 'dic', "preview"];

    let BUILDER = {};

    let API = {};
    API.search = () => new Promise((resolve) => {
    ...
    API.load = (app_id) => new Promise((resolve) => {
    ...
    API.update = (data) => new Promise((resolve) => {
    ...
    API.delete = (app_id) => new Promise((resolve) => {
    ...
    API.timeout = () => new Promise((resolve) => {
        $timeout(resolve);
    });
    ...
    BUILDER.plugin = async () => {
        ...
        $scope.plugin.editor.build = async (targettab, editor) => {
            editor.addCommand(monaco.KeyMod.Alt | monaco.KeyCode.KEY_A, function () {
                ...
                $scope.app.editor.code.change(targettab, prev);
                shortcuts();
            });
            ...
            editor.addCommand(monaco.KeyMod.Alt | monaco.KeyCode.KEY_S, function () {
                ...
                $scope.app.editor.code.change(targettab, next);
                shortcuts();
            });
    ...
    BUILDER.browse = function () {
        ...
        $scope.browse.search = async (val) => {
            ...
            for (var i = 0; i < $scope.browse.data.length; i++) {
                let title = $scope.browse.data[i].package.title.toLowerCase();
                let namespace = $scope.browse.data[i].package.namespace.toLowerCase();
                let app_id = $scope.browse.data[i].package.id.toLowerCase();
                let route = $scope.browse.data[i].package.route.toLowerCase();
                $scope.browse.data[i].hide = true;
                if (namespace.includes(val) || title.includes(val) || app_id.includes(val) || route.includes(val))
