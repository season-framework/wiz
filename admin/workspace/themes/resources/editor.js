var content_controller = function ($scope, $timeout, $sce) {
    ...
    $scope.event.save = function () {
        ...
        $.post('/wiz/admin/workspace/themes/api/update', data, function (res) {
            if (res.code == 200) {
    ...
    $scope.event.delete = function () {
        ...
        if (data.bulk) {
            ...
            $.post('/wiz/admin/workspace/themes/api/delete_bulk', { data: JSON.stringify(sub) }, function (res) {
                $scope.data.selected_delete = null;
                $scope.event.loader(parent, null, true);
                $timeout();
            });
        } else {
            $.post('/wiz/admin/workspace/themes/api/delete', data, function (res) {
                if (isreload) {
                    location.reload();
                    return;
                }
    ...
    $scope.event.create_theme = function (title) {
        $.post('/wiz/admin/workspace/themes/api/rename', { path: BASEPATH, name: title, rename: title }, function (res) {
    ...
    $scope.event.save_name = function () {
        ...
        $.post('/wiz/admin/workspace/themes/api/rename', data, function (res) {
            if (res.code == 402) {
                toastr.error(res.data);
                return;
            }
    ...
    $scope.event.loader = function (item, cb, refresh, init) {
        ...
        $.post('/wiz/admin/workspace/themes/api/tree', { path: item.path, name: item.name, type: item.type }, function (res) {
            if (res.code == 201) {
    ...
    var uploadfile = function (fd) {
        ...
        $.ajax({
            url: "/wiz/admin/workspace/themes/api/upload",
            type: 'POST',
            xhr: function () {
