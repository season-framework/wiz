# Release Notes

All notable changes to the WIZ Framework are documented here.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## 📋 Version Policy

> x.y.z

- **`x` (Major)**: Breaking changes - upgrade not supported
- **`y` (Minor)**: New features - support command upgrade, core function changed, requires server restart
- **`z` (Patch)**: UI updates - support upgrade from web UI, not required server restart

---

## 📦 Current Release - Version 2.4.x

### Latest: [2.4.15] - January 25, 2026

**Highlights:**
- npm package version upgrade
- Python 3.14 compatibility fixes
- Angular 18 upgrade (2.4.9)
- TailwindCSS support (2.4.5)
- WIZ Assistant (GPT) integration (2.4.6)

**[View Full Release Notes →](releases/v2.4.md)**

---

## 📚 Release History

### Version 2.x Series

| Version | Release Date | Highlights | Release Notes |
|---------|-------------|-----------|---------------|
| **2.5.x** | 2026-02-02 ~ (Developing) | CLI Project Tools, Core Optimization | [View Details](releases/v2.5.0.md) |
| **2.4.x** | 2024-03-03 ~ 2026-01-25 | Angular 18, TailwindCSS, AI Assistant | [View Details](releases/v2.4.md) |
| **2.3.x** | 2023-05-24 ~ 2023-12-14 | Bundle structure, Angular 17, Linux service CLI | [View Details](releases/v2.3.md) |
| **2.2.x** | 2023-01-08 ~ 2023-05-03 | IDE overlay menu, shortcut config | [View Details](releases/v2.2.md) |
| **2.1.x** | 2023-01-01 ~ 2023-01-07 | IDE plugin concept, layout changes | [View Details](releases/v2.1.md) |
| **2.0.x** | 2022-10-09 ~ 2022-12-22 | Angular 14, UI/UX redesign, drag & drop | [View Details](releases/v2.0.md) |

### Version 1.x and Earlier

| Version | Release Date | Highlights | Release Notes |
|---------|-------------|-----------|---------------|
| **1.0.x** | 2022-06-10 ~ 2022-07-29 | Clean code, performance improvements | [View Details](releases/v1.0.md) |
| **0.5.x** | 2021-11-28 ~ 2022-05-22 | Plugin storage, programmable API | [View Details](releases/v1.0.md#05x) |
| **0.4.x** | 2021-11-24 | Git flow, workspace structure | [View Details](releases/v1.0.md#04x) |
| **0.3.x** | 2021-11-xx | Socket.IO integration | [View Details](releases/v1.0.md#03x) |
| **0.2.x** | 2021-10-27 ~ 2021-11-10 | Framework structure upgrade | [View Details](releases/v1.0.md#02x) |

---

## 🎯 Quick Links

- **[Latest Stable](releases/v2.4.md)** - Current production release
- **[Migration Guides](docs/en/architecture.md)** - Upgrade between major versions
- **[Contributing](README.md#-contributing)** - How to contribute to the project
- **[Issue Tracker](https://github.com/season-framework/wiz/issues)** - Report bugs or request features

---

## 🔔 Getting Updates

To upgrade to the latest version:

```bash
# Upgrade core framework
pip install season --upgrade

# Upgrade IDE components
wiz ide upgrade
```

---

<div align="center">

**[⬆ Back to Top](#release-notes)** | **[View on GitHub](https://github.com/season-framework/wiz)**

</div>
